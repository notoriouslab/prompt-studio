...
- Duration Range: 3-10s per clip · 30-180s assembled total
- Per-Clip Duration: ≤ 10s — a "spot" in VideoExpress lingo = one I2V clip = one shot in the storyboard. HARD CAP.
- Export Rules: clean unmarked frame, plain unbranded surfaces
- Notes: 本工具最初設計對象。做完整影片企劃，每個 clip 上限 10 秒，總長由使用者選定的時長決定。
```

⚠️ The Export Rules line is deliberately written as a **positive** cleanliness instruction. Earlier versions said `no watermark; no subtitles unless requested`, which pushed the words *watermark* and *subtitles* straight into the generated prompt and made burned-in text more likely, not less (`01-core.md §10 Prompt-side hygiene`). Never reintroduce the negated phrasing here or anywhere else that reaches a prompt field.

**Duration reality check** (2026-08-08 real run): the duration slider is a request, not a guarantee. With lipsync on, the actual clip length follows the generated audio — an 8s setting produced 4.5-6s clips. Plan shot counts from the user's total duration (`01-core.md §6`), and expect the assembled timeline to run slightly shorter than the sum of the requested clip lengths.

## Mode Selection

VideoExpress supports five modes in this distribution:

| User intent | Pick mode |
|-------------|-----------|
| 完整影片企劃 / 多鏡頭 / "幫我做整個 storyboard" / 15s+ | `storyboard` |
| 一個鏡頭 / 短的 / "我只要一個 shot" / 5-15s | `single-shot` |
| 前後對比 / 變化過程 / 從 A 變成 B / 物件轉場 | `first-last` (VE "First Frame, Last Frame" Beta card) |
| 一個人對著鏡頭講一整段 / 講稿為主 / 沒有分鏡 | `avatar` |
| 直式 / 抖音 / Reels / 前 3 秒要抓人 | `short-form` (9:16 locked) |

Default to `storyboard` if the user doesn't specify and the duration suggests > 15s. Within storyboard, also pick an **output mode** (minimal / full / runsheet) per `01-core.md §4` — default `minimal`.

## Platform-Specific Rules (apply while writing; do not print as a section)

- Each Image-To-Video prompt must be ready for direct paste into VideoExpress.ai advanced mode.
- For speaking shots, re-reference the speaker with Actor Alias + a short visual anchor (age, attire, role) before the quoted line — visual anchors stabilize identity across shots better than names alone.
- Keep quoted dialogue under 120 characters per video prompt — split longer lines across shots. In the runsheet output mode the Actor 1 Script cap is 100 characters, enforced by the platform itself.
- Lipsync is implied by the speech-act wrap. **Do NOT append a trailing sentence like `Animate natural lipsync exactly to the quoted words.`** — the wrap pattern already routes the line to lipsync, and the extra sentence is redundant noise VideoExpress's own generator never emits. (The HTML app still carries this legacy line in its platform block; the dogfood-calibrated behaviour here supersedes it.)

## Clarify priority

Clarify these before generating (skip any field the user already gave):

1. **Duration** — the single time authority; everything else derives from it (`01-core.md §6`)
2. **Mode** — storyboard / single-shot / first-last / avatar / short-form (see Mode Selection above)
3. **Output mode** (storyboard only) — minimal (default) / full / runsheet
4. **Domain** — default `narrative-character` if the user defers
5. **MediaType** — default `live` if the user defers
6. **Aspect ratio** — default 16:9; 9:16 for short-form and for the first-last card
7. **Dialogue** — auto-detect from the idea: if the user wrote literal spoken lines (with quotes or 「」), set dialogue=true; else dialogue=false

Bundle the open questions into ONE numbered message; never ask them one at a time.

## Output format (storyboard)

Three output modes, all specified in `02-modes.md §1.2`:

- **minimal** (default) — 2 sections: `# Actor Portrait Image Descriptions` + `# Scenes, Storyboard And Generation Prompts` (`## Shot N` H2 per shot with `**Time:**` / `**Duration:**` / `**Text-To-Image Prompt:**` / `**Image-To-Video Prompt:**` bullets). Fastest path from chat to paste.
- **full** — the 8-section review structure (Project Snapshot → Optional Negative Prompt). Use when the plan will be reviewed or edited by a person before generation.
- **runsheet** — the VideoExpress execution run sheet: frozen Character Bible, three-field routing per scene, settings checklist, How To Run. Use when the user is about to actually drive the platform, by hand or with a browser agent.

Pick one and emit only its sections. Whichever one is chosen, the per-shot craft rules on this page (framing, continuity anchor, dialogue density, closing buffer) apply unchanged.

## Generator cards (UI audit 2026-08-16)

Four ways to generate; all four are prompt-driven and **none of them has a caption or subtitle control** (`01-core.md §10`).

| Card | Fields | Notes |
|------|--------|-------|
| **Create Video From Prompt** | Image Prompt · Video and Audio Prompt · Image Type · enhance-image-prompt · Use Consistent Character · Lipsync HD / Narration / Video Only / gallery share / Advanced Mode | The workhorse. Advanced Mode holds enhance-video-prompt and Manual Video Length (3-10s, default 5). Ticking Lipsync disables the Video and Audio Prompt field and hides Advanced Mode — so set the length **before** ticking Lipsync. |
| **Text To Video** | Simple: Video Prompt + Actor 1 Script + Add Actor 2, with a live character counter. Advanced: one free-form prompt with dialogue inline | Native two-actor lipsync with separate scripts per actor. The card exposes exactly two actor slots, which corroborates the 2-actor soft cap below. |
| **First Frame, Last Frame (Beta)** | First Frame prompt · Last Frame Smart Edit · Video and Audio Prompt | See `02-modes.md §5`. |
| **Export Video** | File name · Quality · Resolution · Format | No burn-in / subtitle option anywhere in export. |

The editor's right rail also lists **Automatic Captions**, but that opens an external third-party service in a new tab — it is not part of VideoExpress generation and nothing is applied by default.
...
When the user's idea implies more than 2 on-screen actors in one shot, you should:
...
- Define each actor's Continuity Anchor **once** — in Character Bible (`full`), Actor Portrait Image Descriptions (`minimal`), or the frozen Character Bible paragraph (`runsheet`).
- Keep the **Actor Alias identical** in every shot, and carry **at least one signature visual trait** into each re-reference. Which one or two traits you cite may vary (`the elderly watchmaker with wire-rimmed glasses` → `the elderly watchmaker with a canvas apron`); that partial paraphrase is what VideoExpress's own generator does and it holds identity fine.
- What breaks identity is **dropping the anchor down to a generic phrase** (`30-year-old Asian male in a dark blue suit` → `Asian man in blue suit`) or swapping every trait at once. VideoExpress re-rolls the appearance when nothing recognizable carries over.
- **Exception — runsheet output mode**: there the Character Bible paragraph is re-pasted **byte-identical** into every scene's Image Prompt, with no paraphrase at all. That mode has no cross-scene context, so verbatim repetition is the only continuity mechanism.
...
VideoExpress's own native generator uses **two distinct generator profiles** depending on content type — confirmed by user dogfood 2026-05-20. You must detect which shape applies and switch calibration accordingly. This is the single most important routing decision after platform selection.
...
### How to detect (apply during the Step 1 clarify in 00-operating-manual.md)
...
VideoExpress has clear strengths and weak spots. Aligning generated prompts with its sweet spot dramatically reduces "the platform didn't render what I asked for" frustration. Source: 3 native-prompt examples from VideoExpress's own native generator (2026-05-19 cold brew live-action + 2026-05-20 two 3D shorts: 父親鐘錶店 + jack/mary 電影院).
...
- **Vary environment / lighting descriptors across shots even when in the same location.** A 6-shot interview that uses the literal phrase `modern television studio` 6 times is a code-smell — you are repeating a placeholder. VideoExpress's native generator describes the same studio differently each shot: `dimly lit high-tech television studio with warm accent lighting`, `high-tech television studio with soft blue and orange ambient lights`, `studio background with soft out-of-focus light leaks`. Each shot gets a slightly different lighting/atmosphere descriptor.

  **Scope limit — vary the wording, never the facts.** All three example descriptors above are the same room at the same hour. This rule does NOT license changing the time of day, the weather, the season, or the location between shots; those are frozen facts (`01-core.md §9`) and a time-of-day arc is decided during clarify, not improvised shot by shot. Drifting from `bright afternoon sunlight` in shot 2 to `warm evening lamplight` in shot 4 inside a single afternoon scene is the most common way this rule gets misapplied.
...
- **Speech-act wrap with 2-word tone combo + neutral accent.** Per `01-core.md §3`: `Actor 1 says in a whispered neutral American accent: "..."` or `Actor 1 ..., and says in a trembling, emotional voice: "..."`.
...
- **Full Continuity Anchor paragraph repeated verbatim inside every shot.** Redundant; use short-form re-references instead (per `01-core.md §2`).

### When the user's idea sounds product-heavy

If the idea is purely product-focused (e.g. "make a 30s cold brew commercial"), you should propose a **storyboard that places a barista's hands or torso in every shot** — even when the shot's narrative purpose is "show the product extracting" — rather than emitting pure-product shots. Mention this trade-off briefly during clarify so the user understands why.
...
**Right (manual rewrite — outcome-only, no object trigger named, abstract physics cause):**
...
| **Walk-and-talk** (邊走邊說，日常題材) | **Reverse dolly at walking pace** — camera ~3m ahead at chest height, retreating at matched speed, subject speaks to lens while walking | `穩定的胸上景反向推軌鏡頭，攝影機位於 Actor 1 前方約三公尺、與她同速後退` | Validated 2026-08-18: walking motion, background parallax and to-camera dialogue all rendered correctly in a single 8s clip |
...
6. **Lock the scale of every prop and set piece** — VE resolves an object name to the largest, most photogenic example it knows. `glass cold brew tower` renders as the metre-tall Kyoto-style lobby rig and dwarfs the actor beside it. Quantify the dimension in centimetres, downgrade the size-inflating noun (`large glass cold brew tower` → `sleek tabletop glass cold brew dripper`), name the surface it stands on, and put the actor's hand on it as a size reference. Repeat the size token in every shot the object appears. Full rule and worked example: `01-core.md §8`.
7. **Use specific reference-tagged settings over generic ones** — generic descriptions (`tropical jungle`, `city street`, `office`) leave VE to fall back to base interpretation. Reference-tagged settings that invoke a recognizable film / cultural / aesthetic register give VE a much stronger prior (it has trained on the reference material).
   ...
   Other valid reference frames you can lean into:
...
Rule: pick the time-of-day arc up-front during clarify step (`30 秒 chase → 黃昏 rescue → 夜間 ending`). Each shot's prompt explicitly states its time-of-day, not just "lighting matches previous" — be specific: `晨間柔光` / `正午直射陽光` / `黃昏暖橘光` / `傍晚 blue hour` / `夜間月光` / `深夜全黑`. Keep the arc monotonic — time never runs backwards inside one sequence.

**Corollary**: with no arc declared during clarify, time of day is a frozen fact and every shot happens within the same hour (`01-core.md §9`). This section licenses a *planned* progression, never incidental drift between shots.
...
| 5 — Hiding behind boulder watching chase recede | (not tested) | not measured | Multi-layer depth (foreground actor + background chase through gap) — see T2I composition rule #2 |
| 6 — Distant chase recedes into jungle | (not tested) | not measured | Wide static, actors as small recurring elements |

## Known limitations (accepted trade-offs)

These VE behaviors emerged across 2026-05-24 v1-v4 dogfood. You should NOT try to fix them via prompt tricks — accept them or warn the user; they're model-level constraints, not prompt-level.
...
- Aspect ratio: 16:9 unless user mentions 直式 / 9:16 / TikTok-style (first-last card and short-form default to 9:16)
- Shot count: derived from the user's total duration via the table in `01-core.md §6` — never a fixed number. Each shot 5-9s by content (10s hard cap); run sheet scenes are ~8s each.
- MediaType: `live` for narrative / interview / vlog / education / product-demo (humans-in-frame); `3d` for 3D-styled animation; `illustration` for editorial-cinemagraph; `2d-animation` for motion-explainer
- visualLock variant per shot: classify the shot's main subject (see `02-modes.md §4.3`) — pure product shots should use the product-subject variant, but **prefer Mixed (actor + product) over pure product whenever possible** per the Strengths section above
- Output mode: `minimal` by default; switch to `runsheet` as soon as the user is actually going to drive the platform (it carries the settings checklist that prevents the identity-breaking mistakes), and to `full` when the plan needs human review first
